<script lang="ts">
	// Stores
	import { translationsStore } from '$lib/stores/translations.store';

	// Props
	export let mostListenedTrackItem: any;
	export let index: number;
	export let choosedItemType: 'artist' | 'track' | 'album' | '';
	export let itemId: string;
	export let showDetailedInfoModalVisible: boolean;
</script>

<button
	class="flex w-full cursor-pointer flex-col gap-3 rounded-lg bg-s-muted/90 p-4 transition-all hover:scale-102 sm:h-40 sm:max-w-lg sm:flex-row sm:items-center sm:gap-4 sm:p-5 lg:h-44 lg:gap-5 lg:p-6"
	on:click={() => {
		choosedItemType = 'track';
		itemId = mostListenedTrackItem.id;
		showDetailedInfoModalVisible = true;
	}}
>
	<picture class="mx-auto shrink-0 sm:mx-0">
		<img
			src={mostListenedTrackItem.album.images[1]?.url}
			srcset="
				{mostListenedTrackItem.album.images[2]?.url} 160w,
				{mostListenedTrackItem.album.images[1]?.url} 320w,
				{mostListenedTrackItem.album.images[0]?.url} 640w
			"
			sizes="(max-width: 640px) 120px, (max-width: 1024px) 112px, 128px"
			alt={mostListenedTrackItem.name}
			class="h-30 w-30 rounded-md object-cover sm:h-28 sm:w-28 lg:h-32 lg:w-32"
			loading="lazy"
		/>
	</picture>

	<div class="flex min-w-0 flex-1 flex-col justify-center gap-1.5 text-center sm:gap-2 sm:text-left">
		<div class="flex flex-col gap-1 sm:gap-1.5">
			<h3 class="text-xs font-semibold uppercase tracking-wide text-t-secondary">
				{index + 2}{$translationsStore.myMusicalProfilePage.myMusicalProfilePageMostListenedItemPlace}
			</h3>

			<p class="line-clamp-1 text-lg font-bold leading-tight text-t-primary sm:text-xl">
				{mostListenedTrackItem.name}
			</p>

			<p class="line-clamp-1 text-sm font-semibold text-t-secondary">
				{#each mostListenedTrackItem.artists as artist, i}
					<span>{artist.name}{i < mostListenedTrackItem.artists.length - 1 ? ', ' : ''}</span>
				{/each}
			</p>
		</div>

		<div class="flex flex-col gap-0.5 text-xs text-t-secondary/70">
			<span class="line-clamp-1 font-medium">{mostListenedTrackItem.album.name}</span>
			<span class="text-xs text-t-secondary/60">{mostListenedTrackItem.album.release_date}</span>
		</div>
	</div>
</button>