...
<button
	class="flex w-64 cursor-pointer flex-col items-center gap-3 rounded-lg bg-s-muted/90 p-4 transition-all hover:scale-102 sm:w-80 sm:flex-row sm:items-start sm:gap-4 sm:p-6 lg:w-96 lg:p-8 xl:w-110"
	on:click={() => {
...
>
	...
	<div class="flex w-full min-w-0 flex-1 flex-col gap-1 text-center sm:text-left lg:gap-2">
		<div class="flex flex-col items-center gap-1 sm:items-start">
			...
			<p
				class="w-full truncate text-base leading-tight font-bold text-t-primary sm:text-lg lg:text-xl xl:text-2xl"
			>
				{mostListenedArtistItem.name}
			</p>

			<p class="text-sm font-semibold text-t-secondary sm:text-base">
